...
if [[ "$POLICY_CONFIG" == "pi05_g1_auto" ]]; then
    ...
    echo -e "  State Space (32 dims):"
    echo -e "    [0:28]  qpos (14 arm + 14 Dex3 hand)"
    echo -e "    [28:31] rpy (roll, pitch, yaw)"
    echo -e "    [31]    yaw_rate"
    echo ""
    echo -e "  Action Space (32 dims):"
    echo -e "    [0:28]  upper_body (14 arm + 14 Dex3 hand)"
    echo -e "    [28]    vx (forward/backward)"
    echo -e "    [29]    vy (strafe left/right)"
    echo -e "    [30]    vyaw (turn)"
    echo -e "    [31]    padding (zero)"
else
    echo -e "${YELLOW}⚠️  Using H1 config for testing${NC}"
    echo -e "    H1 has 26-dim action space (arms + Inspire hands)"
    echo -e "    G1 has 32-dim action space (arms + Dex3 hands + locomotion)"
    echo ""
