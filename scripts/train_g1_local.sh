#!/bin/bash
# Train G1 policy with local LeRobot dataset
#
# Usage:
...
GPU_ID="${GPU_ID:-0}"
ACTION_DIM="${ACTION_DIM:-}"  # Empty: use TrainConfig default (29 for G1)
...
if [ -n "$EPOCH_NUM" ]; then
    ...
    # So we use: exp_name = {task_name}/epoch_{N} to get: checkpoints/pi05_g1_auto/cabinet_bottle/epoch_0/{step}
...
echo "========================================================"
echo "  > Training G1 with local dataset..."
echo "========================================================"
...
if [ -n "$ACTION_DIM" ]; then
    echo "Policy action_dim override: $ACTION_DIM"
fi
echo "========================================================"
...
if [ ! -d "$LEROBOT_DATA_DIR" ]; then
    ...
    echo "Have you run convert_g1_data.sh first?"
    exit 1
fi
